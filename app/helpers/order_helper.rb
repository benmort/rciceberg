module OrderHelper
  ...
  def pin_it_button
    image_url = URI.encode("#{request.scheme}://#{request.host}#{image_path(current_product.image_path)}")
  ...
  def tweet_button
    tweet_text = "I purchased #{current_product.name} purchase <a href='#{tweet_button}'>here</a>!"
  ...
  def encoded_root_url
    raw URI.encode "#{request.scheme}://#{request.host}/order"
  end
